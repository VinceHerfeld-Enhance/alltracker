...
class Net(nn.Module):
    def __init__(
        ...
        self.update_block = RelUpdateBlock(
            self.corr_channel,
            self.num_blocks,
            cdim=dim,
            hdim=hdim,
        ...
        self.register_buffer(
            "time_emb", alltracker.utils.misc.get_1d_sincos_pos_embed_from_grid(self.dim, time_line[0])
        )  # 1,S,C

    def update_seqlen(self, seqlen):
        self.seqlen = seqlen
        time_line = torch.linspace(0, seqlen - 1, seqlen).reshape(1, seqlen, 1)
        self.register_buffer(
            "time_emb", alltracker.utils.misc.get_1d_sincos_pos_embed_from_grid(self.dim, time_line[0])
    ...
    def coords_grid(self, batch, ht, wd, device, dtype):
        coords = torch.meshgrid(
            torch.arange(ht, device=device, dtype=dtype), torch.arange(wd, device=device, dtype=dtype), indexing="ij"
        )
        ...
        if T > 2:  # multiframe tracking
            ...
            for ii, ind in enumerate(indices):
                ...
                flow_predictions, visconf_predictions, flows8, visconfs8, feats8 = self.forward_window(
                    fmap_anchor, fmaps2, visconfs8, iters=iters, flowfeat=feats8, flows8=flows8, is_training=is_training
                )
    ...
    def forward_sliding(self, images, iters=4, sw=None, is_training=False, window_len=None, stride=None):
        ...
        fmap_anchor = self.get_fmaps(images_[:, :1].reshape(-1, 3, H_pad, W_pad), B, 1, sw, is_training).reshape(
            B, C, H8, W8
        )
        ...
        for ii, ind in enumerate(indices):
            ...
            if ii == 0:
                ...
                fmaps2 = self.get_fmaps(images_[:, ara].reshape(-1, 3, H_pad, W_pad), B, S, sw, is_training).reshape(
                    B, S, C, H8, W8
                )
            else:
                flows8 = torch.cat(
                ...
                fmaps2 = torch.cat(
                    [
                        fmaps2[:, stride : stride + S // 2],
                        self.get_fmaps(
                            images_[:, np.arange(ind + S // 2, ind + S)].reshape(-1, 3, H_pad, W_pad),
                            B,
                            S // 2,
                            sw,
                            is_training,
                        ).reshape(B, S // 2, C, H8, W8),
                    ],
                    dim=1,
                )
            ...
            flow_predictions, visconf_predictions, flows8, visconfs8, _ = self.forward_window(
                fmap_anchor, fmaps2, visconfs8, iters=iters, flowfeat=None, flows8=flows8, is_training=is_training
            )
            ...
            full_visconfs[:, to_fill] = (
                unpad_visconf_predictions[-1].reshape(B, S, 2, H, W)[:, -to_fill_sum:].detach().cpu()
            )
    ...
    def forward_window(
        self, fmap1, fmaps2, visconfs8, iters=None, flowfeat=None, flows8=None, sw=None, is_training=False
    ):
        ...
        for itr in range(iters):
            ...
            if self.use_relmotion or self.use_sinrelmotion:
                ...
                if self.use_sinrelmotion:
                    ...
                    motion = (
                        rel_pos_emb_input.reshape(B * S, H8, W8, self.pdim).permute(0, 3, 1, 2).to(dtype)
                    )  # B*S,pdim,H8,W8
                else:
                ...
                if self.use_sinmotion:
                    ...
                    motion = (
                        pos_emb_input.reshape(B * S, H8, W8, self.pdim).permute(0, 3, 1, 2).to(dtype)
                    )  # B*S,pdim,H8,W8
                else:
                    motion = flows8
            ...
            weight_update = 0.25 * self.upsample_weight(flowfeat)
